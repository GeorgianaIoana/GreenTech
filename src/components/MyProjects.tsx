...
const MyProjects = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const scrollProgress = Math.max(0, Math.min((scrollY - sectionTop + sectionHeight) / sectionHeight, 1));
      ...
      if (leftProjects) {
        ...
        for (let i = 0; i < cards.length; i++) {
          ...
          const rotation = scrollProgress * (-10 - i * 5); // Rotate as they move
          const translateX = -scrollProgress * (200 + i * 100); // Push further left
          const translateY = scrollProgress * (i * 30); // Slight vertical spread
      ...
      if (rightProjects) {
        ...
        for (let i = 0; i < cards.length; i++) {
          const card = cards[i] as HTMLElement;
          const rotation = scrollProgress * (10 + i * 5); // Rotate as they move
          const translateX = scrollProgress * (200 + i * 100); // Push further right
          const translateY = scrollProgress * (i * 30); // Slight vertical spread
  ...
  const projects = [
    {
      id: 1,
      title: 'MA2',
      image: 'https://images.pexels.com/photos/957024/forest-trees-perspective-bright-957024.jpeg?auto=compress&cs=tinysrgb&w=600',
      category: 'Web Design',
      number: '01'
    },
    {
  ...
  return (
    <section id="my-projects" className="bg-gradient-to-b from-teal-700 to-teal-50">
      ...
      <div className="py-20 overflow-hidden relative min-h-screen flex items-center">
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          >
            ...
            {/* Main Heading */}
            <h2 className="text-4xl md:text-5xl font-bold text-teal-50 mb-4" style={{ fontFamily: 'Merriweather, serif' }}>
              Some Of
              <br />
            ...
            {/* Description */}
            <p className="text-lg text-gray-700 leading-relaxed max-w-[300px] mx-auto mb-12 max-w-3xl sm:pr-[30px]" style={{ fontFamily: 'Outfit, sans-serif' }}>
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          ...
          >
            {leftProjects.map((project, index) => (
              <div
                key={project.id}
                className="w-80 h-96 rounded-2xl overflow-hidden shadow-2xl transition-transform duration-100 ease-out"
              ...
              >
                <div className="relative w-full h-full">
                  ...
                  <div className="absolute top-6 right-6">
                    <div className="text-white text-lg font-bold">{project.number}</div>
                  </div>
          ...
          >
            {rightProjects.map((project, index) => (
              <div
                key={project.id}
                className={`w-80 h-96 rounded-2xl overflow-hidden shadow-2xl transition-transform duration-100 ease-out ${index === 1 ? '-mt-8' : ''}`}
                style={{ zIndex: 10 + index }}
              >
                <div className="relative w-full h-full">
                  <img 
